...
export default function AuthControl() {
  ...
  async function handleLogoutButtonClick() {
    if(await askForConfirmation('Deseja realmente sair?')) {
      showWaiting
      try{
        // Faz uma requisição para o back-end solicitando a exclusão do cookie com token de autenticação
        await myfetch('/users/logout')

        // Apaga as informações em memoria sobre o usuário autenticado
        setAuthUser(null)
